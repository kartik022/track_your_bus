...
class UserMapState extends State<UserMap> {
  ...
  late LatLng _lastMapPosition; // CLASS MEMBER, MAP OF MARKS
  ...
  pickupRequest(bool value) async {
    ...
    if(value == true) {
      ...
      firestore.collection(widget.institute).doc(widget.bus).collection('Users').doc(widget.uid).update({
        'position': point.data,
        'pickup': value,
      });
    } else {
  ...
  Widget build(BuildContext context) {
    return _center == null ? Scaffold(
      body: Center(
        ...
        appBar: AppBar(
          title: Text("Track_your_bus"),
        ),
        drawer: DrawerWidget(name: widget.name, userID: widget.userID, institute: widget.institute,),
        body: SlidingUpPanel(
          panel: StreamBuilder<QuerySnapshot>(
            stream: firestore.collection(widget.institute.toString()).doc(widget.bus.toString()).collection("Driver").snapshots(),
            ...
            child: Center(
              ...
              Column(
                ...
                children: <Widget>[
                  ...
                  StreamBuilder(
                    stream: firestore.collection(widget.institute).doc(widget.bus).collection('Users').doc(widget.uid).snapshots(),
                    builder: (context, snap) {
                      if(snap.hasData) {
