...
class FindBus extends StatefulWidget {
  ...
  FindBus({Key? key, required this.institute}) : super(key: key);
  @override
...
class FindBusState extends State<FindBus> {
  ...
  Future<List<DocumentSnapshot>> getBusList() async {
    var firestore = FirebaseFirestore.instance;
    QuerySnapshot qn = await firestore.collection(widget.institute).get();
    return qn.docs;
  }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, index){
                    ...
                    return ListTile(
                      ...
                      title: Text("Bus Number : " + id),
                      trailing: Icon(Icons.arrow_forward),
                      onTap: () {
                        Navigator.push(context, PageRouteBuilder(
                            pageBuilder: (BuildContext context, _, __) {
                              return new BusDetail(institute: widget.institute, bus: id,);
                            },
...
class BusDetail extends StatefulWidget {
  final String institute;
  final String bus;
  BusDetail({Key? key,required this.institute,required this.bus}) : super(key: key);
  @override
  State<StatefulWidget> createState() {
...
class BusDetailState extends State<BusDetail> {
  ...
  late GeoFirePoint _driverCurrentLocation;
    ...
    return Scaffold(
      ...
      body: SlidingUpPanel(
        panel: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection(widget.institute.toString()).doc(widget.bus.toString()).collection("Driver").snapshots(),
